...
| `reproducible` | `false` | ajoute `--reproducible` aux deux builds qui **poussent** (pas au check de PR, qui est en `--no-push`). Normalise les horodatages des couches : sans lui, le mtime de `/app` est réécrit par chaque `COPY` de l'étage runtime et fait changer le sha256 de toutes les couches applicatives, `node_modules` (178 Mo) comprise — le registre en stocke une copie **par build**. Mesuré le 2026-09-03 sur `bot-twitch-api` : 205 copies de cette seule couche, ~36 Go pour un contenu identique. ⚠️ **opt-in** : kaniko réécrit alors toutes les couches en mémoire, et il part déjà en OOMKilled à `4Gi` sur une image lourde (`gaspezia-voice`, 2026-08-30) — prévoir de remonter `kanikoMemLimit` / `kanikoMigrateMemLimit`. Ne suffit pas seul côté dépôt : `--reproducible` ne touche jamais au **contenu** d'un fichier, or pnpm réécrit deux dates dans `node_modules/.modules.yaml` et `.pnpm-workspace-state-v1.json` à chaque install (cf. le Dockerfile de `bot-twitch-api`) |
